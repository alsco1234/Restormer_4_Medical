...
# second step
# model_restoration.load_state_dict(torch.load('./pts/0_0fine_tuned_model11.pt'))
...
with tf.device('/device:GPU:0'):
    ...
    print("Train_Patches.shape : ", X_Train_Patches.shape, "Val_Patches.shape : ", X_Val_Patches.shape) # (n, 256, 256, 3)
...
optimizer = torch.optim.AdamW(model_restoration.parameters(), lr=3e-5, betas=(0.9, 0.999), weight_decay=1e-3) # lr = 3e-4, betas=(0.9, 0.999), weight_decay=1e-4                      

num_epochs = 50
# Define the cosine annealing scheduler
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=num_epochs, eta_min=1e-7) # eta_min = 1e-6
...
for epoch in range(num_epochs):
    ...
    torch.save(model_restoration.state_dict(), './pts/'+str(step)+'_'+str(epoch)+'fine_tuned_model11.pt')

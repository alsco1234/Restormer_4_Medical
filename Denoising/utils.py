...
def concat_h(img1, img2, overlap_size):
...
def concat_v(img1, img2, overlap_size):
    for x in range(0, img1.shape[0]):
        for y in range(0, img1.shape[1]):
...
if __name__ == "__main__":
    test_img = cv2.imread('./data/test/origin/Chest1.png', cv2.COLOR_BGR2GRAY)
    
    patches = divide_patches(test_img, overlap_size=85)
    
    cv2.imshow("patches[0][0]", patches[0][0])
    cv2.waitKey(0)
    
    merged_img = merge_patches(patches, overlap_size=85)
